...
$(document).ready(function(){
	...
	$('a.poplight').click(function() {
		...
		$('#' + popID).fadeIn().css({ 'width': Number( popWidth ) }).prepend('<a href="#" title="Закрыть" class="close-modal"></a>');
	...
	$(document).on('click', 'a.close-modal, #fade', function() { //закрытие по клику вне окна, т.е. по фону...
    $('#fade , .popup_block').fadeOut(function() {
        $('#fade, a.close-modal').remove();
        $('body').css('overflow','');  //плавно исчезают 
    });   
    return false;    
   });
